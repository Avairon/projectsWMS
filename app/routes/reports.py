...
from app.tables import create_projects_table, create_tasks_table, create_subtasks_table
...
@reports_bp.route('/api/reports/subtasks')
@login_required
def get_subtasks_report():
    """
    API endpoint для получения данных отчета по подзадачам
    
    Параметры запроса:
        task_id: ID задачи для фильтрации подзадач
        search: строка для поиска по текстовым полям
        start_date: начальная дата для фильтра (формат DD.MM.YYYY)
        end_date: конечная дата для фильтра (формат DD.MM.YYYY)
    """
    ...
    # Получаем все подзадачи из всех задач
    all_subtasks = []
    for task in tasks:
        subtasks = task.get('subtasks', [])
        for subtask in subtasks:
            subtask_with_task = subtask.copy()
            subtask_with_task['task_id'] = task.get('id')
            subtask_with_task['task_title'] = task.get('title', '')
            subtask_with_task['project_id'] = task.get('project_id')
            all_subtasks.append(subtask_with_task)
    
    # Создаем таблицу
    table = create_subtasks_table(all_subtasks, users, tasks)
    
    # Применяем фильтр по task_id, если указан
    task_id = request.args.get('task_id')
    if task_id:
        table.filter_by_task_id(task_id)
    ...
    if start_date and end_date:
        table.filter_by_date_range('planned_date', start_date, end_date)
    elif start_date:
        table.filter_by_date('planned_date', start_date)
    
    # Возвращаем данные в формате для фронтенда
    return jsonify(table.to_dict())


@reports_bp.route('/api/reports/subtasks/download')
@login_required
def download_subtasks_report():
    """Download endpoint for subtasks report in Excel format"""
    if current_user.role != 'admin':
        return jsonify({'error': 'У вас нет прав доступа к этой странице'}), 403
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Отчет по подзадачам"
    
    # Define headers (с нумерацией)
    headers = ["#", "Задача", "Подзадача", "Плановая дата", "Дата выполнения", "Статус", "Отчет", "Файл"]
    ...
    for row_num, item in enumerate(filtered_data, 2):
        ws.cell(row=row_num, column=1, value=row_num - 1)  # Номер строки
        ws.cell(row=row_num, column=2, value=item.get('task_title', ''))
        ws.cell(row=row_num, column=3, value=item.get('title', ''))
        ws.cell(row=row_num, column=4, value=item.get('planned_date', ''))
        ws.cell(row=row_num, column=5, value=item.get('completed_date', ''))
        ws.cell(row=row_num, column=6, value='Выполнена' if item.get('completed') else 'Не выполнена')
        ws.cell(row=row_num, column=7, value=item.get('report', '')[:50] + '...' if len(item.get('report', '')) > 50 else item.get('report', ''))
        ws.cell(row=row_num, column=8, value=item.get('file', {}).get('filename', '') if item.get('file') else '')
    ...
    # Информация о фильтрах внизу
    if search_query or task_id or start_date:
        info_row = len(filtered_data) + 3
        ws.merge_cells(start_row=info_row, start_column=1, end_row=info_row, end_column=8)
        info_cell = ws.cell(row=info_row, column=1)
        info_text = f"Всего записей: {table.get_total_count()}, Отображено: {table.get_filtered_count()}"
        if task_id:
            info_text += f" | Задача: {task_id}"
        if search_query:
            info_text += f" | Поиск: '{search_query}'"
        if start_date and end_date:
            info_text += f" | Фильтр по дате: от {start_date} до {end_date}"
        elif start_date:
            info_text += f" | Фильтр по дате: {start_date}"
        info_cell.value = info_text
        info_cell.font = Font(italic=True, size=9)
        info_cell.fill = PatternFill(start_color="E0E0E0", end_color="E0E0E0", fill_type="solid")
        info_cell.alignment = Alignment(horizontal="left")
    ...
    timestamp = int(time.time())
    filename = f'report_subtasks_{current_user.id}_{timestamp}.xlsx'

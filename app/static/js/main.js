...
// Функция переключения видимости команды проекта
function toggleTeam() {
...
// Функция переключения истории изменений задачи
...
function showEditSubtaskForm(subtaskId) {
    ...
    const plannedDateElement = subtaskElement.querySelector('.planned-date');
    const reportElement = subtaskElement.querySelector('.report-text');
    
    const titleText = titleElement.textContent;
    const plannedDateText = plannedDateElement ? plannedDateElement.textContent.replace('Запланировано: ', '') : '';
    const reportText = reportElement ? reportElement.textContent : '';
    
    // Создаем форму редактирования с возможностью добавления отчета
    ...
    editForm.innerHTML = \`
        ...
        <div class="form-group">
            ...
            <input type="text" id="edit-subtask-date-\${subtaskId}" class="form-control" value="\${plannedDateText}" placeholder="ДД.ММ.ГГГГ">
        </div>
        <div class="form-group">
            <label for="edit-subtask-report-\${subtaskId}">Отчет по подзадаче</label>
            <textarea id="edit-subtask-report-\${subtaskId}" class="form-control" rows="3" placeholder="Введите отчет по подзадаче">\${reportText}</textarea>
        </div>
        <div class="form-group">
            <label for="edit-subtask-file-\${subtaskId}">Файл отчета (необязательно)</label>
            <input type="file" id="edit-subtask-file-\${subtaskId}" class="form-control">
        </div>
        <div class="form-actions">
            <button type="button" class="btn btn-success" onclick="updateSubtaskWithReport('\${subtaskId}')">Сохранить</button>
...
// Функция обновления подзадачи с отчетом
async function updateSubtaskWithReport(subtaskId) {
    ...
    const report = document.getElementById(\`edit-subtask-report-\${subtaskId}\`).value.trim();
    const fileInput = document.getElementById(\`edit-subtask-file-\${subtaskId}\`);
    ...
    // Если есть файл или отчет, используем formData для отправки
    if (file || report) {
        ...
        // Если нет файла и отчета, используем обычный запрос
        try {
            const response = await fetch(\`/task/\${currentTaskId}/subtask/\${subtaskId}\`, {
                method: 'PUT',
                headers: {

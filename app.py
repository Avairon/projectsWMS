...
def can_access_project(project_id):
    ...
    if current_user.role in ['manager', 'curator'] and (project.get('manager_id', '') == current_user.id or project.get('supervisor_id', '') == current_user.id):
        return True
...
def generate_token_route():
    ...
    if role not in ['admin', 'manager', 'curator', 'worker']:
